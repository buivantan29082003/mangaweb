<?php

namespace App\Http\Controllers\User;

use App\Models\RegistrationPlan;
use App\Models\Transaction;
use App\Response\DataResponse;
use App\Response\PageResponse;
use App\Services\Common\ValidateType;
use App\Services\User\FactoryPaymentService;
use App\Services\User\PlanService;
use App\Services\User\RegistrationService;
use App\Services\User\TransactionService;
use DateTime;
use Illuminate\Http\Request;
use Illuminate\Routing\Controller;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Log;

class RegistrationController extends Controller
{



    private FactoryPaymentService $factoryPayment;

    private PlanService $planService;

    private RegistrationService $registrationService;

    private TransactionService $transactionService;

    public function __construct(FactoryPaymentService $factoryPayment, PlanService $planService, RegistrationService $registrationService, TransactionService $transactionService)
    {
        $this->factoryPayment = $factoryPayment;
        $this->planService = $planService;
        $this->registrationService = $registrationService;
        $this->transactionService = $transactionService;
    }

    public function getPaymentType()
    {
        return response()->json(new DataResponse("success", [["id" => "vnpay", "name" => "VNPay", "icon" => "data:image/png;base64,"]], false));
    }

    public function getAll(Request $request){
        $optionRequest = $request->query(); 
        $user=Auth::guard("api")->user(); 
        $currentPage = ValidateType::validateInt($optionRequest['currentPage']?? null ,1);
        $planId =ValidateType::validateInt($optionRequest['planId']?? null ,null);
        $data=$this->registrationService->getAll(["*"],["transaction","plan"],$planId,$currentPage, $user->id);
        return new DataResponse("success",new PageResponse($data));
    }


    public function createUrlPayment(string $paymentType, int $planId, Request $request)
    {
        $paymentService = $this->factoryPayment->loadPaymentService($paymentType);
        if ($paymentService != null) {
            $plan = $this->planService->getById($planId, ["*"], []);
            if ($plan != null) {
                $transaction = new Transaction();
                $transaction->createdDate = now();
                $transaction->transactionId = null;
                $transaction->createdDate = now();
                $transaction->bankCode = "vnpay";
                $transaction->accountId = $request->user->id;
                $transaction->totalAmount = $plan->fee;
                $this->transactionService->insert($transaction);
                Log::info($transaction->id);
                $urlPayment = $paymentService->createUrl($plan, $transaction->id);
                Log::info($urlPayment);
                return response()->json(new DataResponse("success", $urlPayment, false), 200);
            }
            return response()->json(new DataResponse("Not find plan you need to reggister", null, true), 400);
        }
        return response()->json(new DataResponse("Not match payment method", null, true), 400);
    }

    public function callBackPayment(Request $request)
    {
        $vnp_Params = $request->all();
        $txnRef = $vnp_Params['vnp_TxnRef'] ?? null; 
        $orderType = $vnp_Params['vnp_OrderType'] ?? null; 
        $orderInfo = $vnp_Params['vnp_OrderInfo'] ?? null; 
        $transactionId   = $request->input('vnp_TransactionNo'); 
        $bankCode       = $request->input('vnp_BankCode');
        $responseCode = $vnp_Params['vnp_ResponseCode'] ?? null;
        $vnp_SecureHash = $vnp_Params['vnp_SecureHash'] ?? '';
        $vnp_HashSecret = config('vnpay.secret_key');
        unset($vnp_Params['vnp_SecureHash']);
        unset($vnp_Params['vnp_SecureHashType']);
        ksort($vnp_Params);
        $hashData = "";
        foreach ($vnp_Params as $key => $value) {
            $hashData .= urlencode($key) . "=" . urlencode($value) . "&";
        }
        $hashData = rtrim($hashData, "&");
        $secureHashCheck = hash_hmac('sha512', $hashData, $vnp_HashSecret);
        if ($responseCode === "00") {
            $plan = $this->planService->getById((int)$orderInfo, ["*"], []);
            $transaction = $this->transactionService->getById((int)$txnRef);
            $currentPlan = $this->registrationService->getCurrentPlan($transaction->accountId);
            $registration = new RegistrationPlan();
            $d = new DateTime();
            $registration->registrationDate = new DateTime();
            $d->modify("+{$plan->months} months");
            if ($currentPlan != null) {
                if($currentPlan->planId==$plan->id){
                    $now = new DateTime();
                    $oldExpire = new DateTime($currentPlan->expireDate);
                    $diff = $now->diff($oldExpire);
                    $remainingMinutes = ($diff->days * 24 * 60) + ($diff->h * 60) + $diff->i;
                    if ($remainingMinutes > 0) {
                        $d->modify("+{$remainingMinutes} minutes");
                    } 
                }
                $this->registrationService->endedPlan($currentPlan->id);
            }
            $registration->expireDate = $d;

            $this->transactionService->completeTransaction($transaction->id, $transactionId);
            $registration->planId = $plan->id;
            $registration->accountId = $transaction->accountId;
            
            $registration->isEnded = 0;
            $registration->transactionId = $transaction->id;
            $this->registrationService->insert($registration);
            return response()->json(new DataResponse("success", null, false));
        } else {
            return response()->json([
                "message" => "Thanh toán thất bại",
                "userId" => $txnRef,
                "planId" => $orderInfo,
                "orderType" => $orderType
            ]);
        }
    }
}
